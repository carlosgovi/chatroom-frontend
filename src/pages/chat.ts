...
///declaro el tipo mensaje donde voy a guardar los datos del mensaje
type Messages = {
  from: string;
  message: string;
};
export class Chat extends HTMLElement {
  connectedCallback() {
    ///me suscribo al state
    state.subscribe((): any => {
      //guardo el state avnterior
      const currentState = state.getState();
      //piso this menssage con los mensajes del state q son los que trae del rtdb
      this.messages = currentState.messages;
      //hago un nuevo render para ver los cambios
      this.render();
    });
    this.render();
  }
  ...
  addListeners() {
    const form: any = document.querySelector(".submit-message");
    form.addEventListener("submit", (e: any) => {
      e.preventDefault();
      console.log("dato del input ", e.target["new-message"].value);
      //enpujo el messaje el el state
      state.pushMessaje(e.target["new-message"].value);
      form.reset();
...
.emojis:active {
    ...
    this.innerHTML = `
      ...
      ${this.messages
        .map((m) => {
          if (m.from == state.getState().nombre) {
      ...
      <input class="input textarea" type="text" name="new-message" />
      <div class="emojis">
      <button class="emojis" type="submit">Enviar</button>
      </div>
